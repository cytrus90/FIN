        ...
        for transaction:AnyObject in transactions {
            if let trans = transaction as? SKPaymentTransaction {
                ...
                case .purchased:
                    ...
                    nc.post(name: Notification.Name("purchaseDone"), object: nil) // reload graphsVC
                    break
                case .failed:
                    log("Product purchase failed")
                    SKPaymentQueue.default().finishTransaction(transaction as! SKPaymentTransaction)
                    nc.post(name: Notification.Name("purchaseFailed"), object: nil) // reload finVC
                    break
                case .restored:
                    log("Product restored")
                    SKPaymentQueue.default().finishTransaction(transaction as! SKPaymentTransaction)
                    nc.post(name: Notification.Name("purchaseRestored"), object: nil) // reload listMasterVC
                    break
                default:
                    break
                }}}
